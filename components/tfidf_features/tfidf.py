...
def main():
    ...
    deploy_text = deploy_df[args.text_column].fillna("").astype(str)

    # IMPORTANT: fit ONLY on training split, transform the rest
    vec = TfidfVectorizer(
    ...
    X_deploy = vec.transform(deploy_text)  # transform only, never fit
    ...
    out_deploy = pd.DataFrame(X_deploy.toarray(), columns=feat_cols).astype("float32")
